...
const towerInfo = towerInfoContainer.children.namedItem(
  'content'
) as HTMLElement
//
const towerTitle = towerInfo.children.namedItem('title') as HTMLHeadingElement
const towerInfoContent = towerInfo.children.namedItem(
  'content'
) as HTMLDivElement
//
const towerImage = towerInfoContent.children.namedItem(
  'image'
) as HTMLDivElement
//
const towerHealthBar = towerInfoContent.children.namedItem(
  'health-bar'
) as HTMLDivElement
const healthBar = towerHealthBar.querySelector(
  'progress'
) as HTMLProgressElement
const healthValue = towerHealthBar.querySelector('span') as HTMLSpanElement
//
const towerDescription = towerInfoContent.children.namedItem(
  'description'
) as HTMLParagraphElement
const showMoreButton = towerInfo.querySelector('#show-more') as HTMLSpanElement
//
const towerStats = towerInfoContent.children.namedItem(
  'stats'
) as HTMLDivElement
const statsTable = towerStats.children.namedItem(
  'stats-table'
) as HTMLTableElement
const towerLevel = statsTable.querySelector(
  '[name="level"]'
) as HTMLTableCellElement
const towerHealth = statsTable.querySelector(
  '[name="health"]'
) as HTMLTableCellElement
const towerAttackSpeed = statsTable.querySelector(
  '[name="attack-speed"]'
) as HTMLTableCellElement
const towerDamage = statsTable.querySelector(
  '[name="damage"]'
) as HTMLTableCellElement
const towerCooldown = statsTable.querySelector(
  '[name="cooldown"]'
) as HTMLTableCellElement

const nextTowerLevel = statsTable.querySelector(
  '[name="next-level"]'
) as HTMLTableCellElement
const nextTowerHealth = statsTable.querySelector(
  '[name="next-health"]'
) as HTMLTableCellElement
const nextTowerAttackSpeed = statsTable.querySelector(
  '[name="next-attack-speed"]'
) as HTMLTableCellElement
const nextTowerDamage = statsTable.querySelector(
  '[name="next-damage"]'
) as HTMLTableCellElement
const nextTowerCooldown = statsTable.querySelector(
  '[name="next-cooldown"]'
) as HTMLTableCellElement

const towerUpgradeCost = towerStats.children.namedItem(
  'upgrade-cost'
) as HTMLSpanElement
//
export const upgradeTowerButton = towerInfo.children.namedItem(
  'upgrade-button'
) as HTMLButtonElement
const closeButton = towerInfo.children.namedItem(
  'close-button'
) as HTMLButtonElement
const backButton = towerInfo.children.namedItem(
  'back-to-shop-button'
) as HTMLButtonElement

showMoreButton.onclick = () => {
  towerDescription.classList.toggle('extended')
  showMoreButton.innerText = towerDescription.classList.contains('extended')
    ? 'Show less...'
    : 'Show more...'
}
...
const updateTable = (tower: Tower | Ally) => {
  ...
  statsTable
    .querySelectorAll('td:last-of-type')
    .forEach(td => td.classList.remove('hidden'))
  statsTable
    .querySelectorAll('th:last-of-type')
    .forEach(th => th.classList.remove('hidden'))
  ;(
    statsTable.querySelector('th:nth-of-type(2)') as HTMLTableCellElement
  ).innerText = 'CURRENT'
  ...
  if (tower instanceof Ally && 'particles' in tower && tower.particles)
    tower.particles.resume()
}
...
export function toggleTowerInfo(tower?: Tower | Ally | AllyType) {
  if (!tower && towerInfoContainer.classList.contains('visible'))
    hideTowerInfo()
  if (tower instanceof Tower || tower instanceof Ally) {
    ...
    statsTable
      .querySelectorAll('th:last-of-type')
      .forEach(th => th.classList.add('hidden'))
    ;(
      statsTable.querySelector('th:nth-of-type(2)') as HTMLTableCellElement
    ).innerText = 'VALUE'
